...
describe("createDeviceInstance", () => {
  it("fills in the default device settings for a new instance", () => {
    ...
    expect(device.screenZoom).toBe(100);
    expect(device.screenOffsetX).toBe(0);
    expect(device.screenOffsetY).toBe(0);
    expect(device.showFrame).toBe(true);
    expect(device.statusBar.enabled).toBe(false);
    expect(device.statusBar.time).toBe("9:41");
...
describe("ensureDeviceInstances", () => {
  ...
  it("preserves an explicitly empty device list", () => {
    const { devices, activeDeviceId } = ensureDeviceInstances({
      devices: [],
      activeDeviceId: "",
    });
