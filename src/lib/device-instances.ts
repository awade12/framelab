...
import { normalizeStatusBar } from "./status-bar";
import type {
  DeviceColor,
  DeviceInstance,
  DeviceSpec,
...
export const createDeviceInstance = (
  ...
  return {
    ...
    screenZoom: overrides.screenZoom ?? 100,
    screenOffsetX: overrides.screenOffsetX ?? 0,
    screenOffsetY: overrides.screenOffsetY ?? 0,
    showFrame: overrides.showFrame ?? true,
    statusBar: normalizeStatusBar({
      ...overrides.statusBar,
    }),
  };
};
    ...
    ? screenshot.devices
        ...
        .map((device) =>
          createDeviceInstance({
            ...
            screenZoom:
              typeof device.screenZoom === "number" ? device.screenZoom : undefined,
            screenOffsetX:
              typeof device.screenOffsetX === "number"
                ? device.screenOffsetX
                : undefined,
            screenOffsetY:
              typeof device.screenOffsetY === "number"
                ? device.screenOffsetY
                : undefined,
            showFrame:
              typeof device.showFrame === "boolean" ? device.showFrame : undefined,
            statusBar:
              typeof device.statusBar === "object" && device.statusBar !== null
                ? normalizeStatusBar(device.statusBar as Partial<DeviceInstance["statusBar"]>)
                : undefined,
          }),
        )
    : [];
