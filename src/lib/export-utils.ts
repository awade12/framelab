...
import { getExportScale } from "./export-sizes";
...
const renderDeviceToOffscreen = async (
  ...
  if (statusBar) {
    drawStatusBar(ctx, screenX, screenY, screenW, screenH, statusBar, scaleX);
  }
...
const renderFramelessScreenToOffscreen = async (
  widthPx: number,
  heightPx: number,
  screenRadius: number,
  screenshotSrc: string | null,
  ...
  const offscreen = document.createElement("canvas");
  offscreen.width = Math.ceil(widthPx);
  offscreen.height = Math.ceil(heightPx);
  const ctx = offscreen.getContext("2d")!;

  ctx.fillStyle = "#1c1c1e";
  ctx.beginPath();
  ctx.roundRect(0, 0, widthPx, heightPx, screenRadius);
  ctx.fill();
  ...
  if (statusBar) {
    drawStatusBar(ctx, 0, 0, widthPx, heightPx, statusBar, 1);
  }

  return offscreen;
};

const drawFramelessDeviceInstance = async (
  ctx: CanvasRenderingContext2D,
  ...
  const [innerRadiusXPct, innerRadiusYPct] = parseFrameRadius(
    selectedDevice.frameRadius.inner,
  );
  const screenRadius = Math.min(
    deviceWidthPx * innerRadiusXPct,
    deviceHeightPx * innerRadiusYPct,
  );
  ...
  if (device.style === "3d") {
    ...
    const offscreen = await renderFramelessScreenToOffscreen(
      deviceWidthPx,
      deviceHeightPx,
      screenRadius,
      device.screenshotSrc,
      device.screenZoom ?? 100,
      device.screenOffsetX ?? 0,
      device.screenOffsetY ?? 0,
      device.statusBar,
    );
  ...
  ctx.fillStyle = "#1c1c1e";
  ctx.beginPath();
  ctx.roundRect(deviceX, deviceY, deviceWidthPx, deviceHeightPx, screenRadius);
  ctx.fill();
...
const drawDeviceInstance = async (
  ...
  if (device.showFrame === false) {
    await drawFramelessDeviceInstance(ctx, canvas, device, scaleX, renderX);
    return;
  }
...
export const renderExportFiles = async ({
  screenshots,
  exportSize,
  exportQuality = 2,
  previewDimensions,
  ...
  report({
    progress: 4,
    phase: "Preparing export pipeline",
    detail: "Initializing canvas renderer and quality settings…",
    screenshotIndex: 0,
  ...
  report({
    progress: 10,
    phase: "Loading fonts & assets",
    detail: "Waiting for web fonts and embedded images to finish loading…",
    screenshotIndex: 0,
    screenshotTotal: screenshots.length,
  });

  const quality = getExportScale(exportSize, exportQuality);
  const exportWidth = Math.round(exportSize.width * quality);
  const exportHeight = Math.round(exportSize.height * quality);
  const exportedFiles: { name: string; data: string }[] = [];
  const renderProgressStart = 14;
  const renderProgressEnd = 86;
  const renderSpan = renderProgressEnd - renderProgressStart;

  for (let i = 0; i < screenshots.length; i++) {
    const screenshot = screenshots[i];
    const screenshotProgress =
      renderProgressStart +
      (renderSpan * (i + 0.15)) / Math.max(screenshots.length, 1);

    report({
      progress: screenshotProgress,
      phase: `Rendering screenshot ${i + 1}`,
      detail: "Drawing background, text, devices, and custom assets…",
      screenshotIndex: i + 1,
      screenshotTotal: screenshots.length,
    });
    const qualitySuffix = quality > 1 ? `@${quality}x` : "";
    const filename = `appstore-screenshot-${i + 1}${qualitySuffix}.png`;
    ...
    const previewWidth = Math.max(previewDimensions.width, 1);
    ...
    report({
      progress:
        renderProgressStart +
        (renderSpan * (i + 1)) / Math.max(screenshots.length, 1),
      phase: `Finished screenshot ${i + 1}`,
      detail: "Composited layers and encoded PNG buffer…",
      screenshotIndex: i + 1,
      screenshotTotal: screenshots.length,
    });
  }
  ...
  report({
    progress: 88,
    phase: "Finalizing renders",
    detail: `${exportedFiles.length} PNG${exportedFiles.length === 1 ? "" : "s"} ready for packaging…`,
...
export const exportScreenshots = async (options: ExportOptions) => {
  const files = await renderExportFiles(options);
