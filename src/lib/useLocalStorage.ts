...
const STORAGE_KEY = "framelab-editor-state";
...
export const loadPersistedState = (): PersistedEditorState | null => {
  try {
    ...
    if (parsed.version > CURRENT_VERSION) {
      console.warn(
        `Editor state version ${parsed.version} is newer than supported ${CURRENT_VERSION}.`,
      );
      return null;
    }

    const migrated =
      parsed.version < CURRENT_VERSION ? migratePersistedState(parsed) : parsed;
